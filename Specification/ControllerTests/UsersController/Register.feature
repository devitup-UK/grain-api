...
Scenario Outline: Allow an anonymous user to join the system as a registered user.
	Given I have prepared a "POST" request
	And the prepared request has a RegisterModel parameter
	And the prepared RegisterModel parameter in the request has a "FirstName" parameter whose value is "<FirstName>"
	And the prepared RegisterModel parameter in the request has a "LastName" parameter whose value is "<LastName>"
	And the prepared RegisterModel parameter in the request has a "Username" parameter whose value is "<Username>"
	And the prepared RegisterModel parameter in the request has a "EmailAddress" parameter whose value is "<EmailAddress>"
	And the prepared RegisterModel parameter in the request has a "Password" parameter whose value is "<Password>"
	And the prepared RegisterModel parameter in the request has a "ConfirmPassword" parameter whose value is "<Password>"
...
Scenario Outline: Disallow a user to join the system when an existing user already exists with a specific username.
	Given I have registered a user with the details (FirstName: "<FirstName>", LastName: "<LastName>", Username: "<Username>", Email Address: "<EmailAddress>", Password: "<Password>", ConfirmPassword: "<Password>")
...
Scenario Outline: Disallow a user to join the system when submitting Password and ConfirmPassword do not match.
	...
	And the prepared RegisterModel parameter in the request has a "EmailAddress" parameter whose value is "<EmailAddress>"
	And the prepared RegisterModel parameter in the request has a "Password" parameter whose value is "<Password>"
	And the prepared RegisterModel parameter in the request has a "ConfirmPassword" parameter whose value is "<ConfirmPassword>"
	When I submit the prepared request to "/users/register"
	Then the endpoint will return a status code of "400"
